...
class MainPage:
    def __init__(self, root):
        ...
        search_button = ttk.Button(navbar, text="Search", width=10,
                                   command=lambda: self.SearchButton_clicked(value_inside.get()))
        search_button.grid(column=1, row=0, padx=5, pady=5, sticky="w")

        # Autodetect of location
        autodetect_button = ttk.Button(navbar, text="Autodetect location",
                                       command=lambda: self.AutoDetectButton_clicked())
    ...
    def InsertData(self, currentWeather: WeatherForecastModel, city_name):
        # Current weather
        ttk.Label(self.current_weather, text=f"Current weather in {city_name}", font=("Arial", 15)).grid(column=0,
                                                                                                         row=0, padx=5,
                                                                                                         pady=5,
                                                                                                         sticky="w")
        ...
        # Hourly forecast
        hourly_forecast = [item for item in currentWeather.hourlyForecast if item.date.hour > currentWeather.date.hour]
        for i, item in enumerate(hourly_forecast[0:12]):
