...
class CurrentWeatherItem(ttk.Frame):
    def __init__(self, parent, entire_forecast):
        ...
        main_text= "Today is " + entire_forecast.date.strftime("%A\n%d %b %Y") + "\n" + "Time is " + entire_forecast.date.strftime("%H:%M")
        label_text = str(entire_forecast.temperature) + "°C" + "\n" + "Humidity: " + str(entire_forecast.humidity) + "%" + "\n" + "Rain: " + str(entire_forecast.rain) + "mm" + "\n" + "Wind speed: " + str(entire_forecast.wind_speed) + "km/h" + "\n" + "Wind direction: " + obtainWindDirection(entire_forecast.wind_direction)
